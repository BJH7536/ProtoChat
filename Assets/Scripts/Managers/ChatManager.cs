...
public class ChatManager : MonoBehaviour
{
    ...
    private void Start()
    {
        TCPClient.textdelegate += new MessageDelegate(Chat);
        TCPClient.imagedelegate += new ImageDelegate(Chat1);
    }
    ...
    public void Chat(bool isSend, string text, string user, Texture texture)
    {
        if (text.Trim() == "") return;
    ...
    public void Chat1(bool isSend, string text, string user, string Path)
    {
        if (text.Trim() == "") return;
        ...
        Area.transform.SetParent(ContentRect.transform, false);
        Area.BoxRect.sizeDelta = new Vector2(280, Area.BoxRect.sizeDelta.y);
        loadImgFromPath(Area.Image, Path);

        Fit(Area.Image.rectTransform);
        Fit(Area.BoxRect);                              // 텍스트만 대입하면 버그가 있다. 이를 해결하기 위해 Fit()
    ...
    void loadImgFromPath(Image image, string fileName)
    {
